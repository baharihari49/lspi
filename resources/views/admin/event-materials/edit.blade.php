...
@section('content')
    <form action="{{ route('admin.events.materials.update', [$event, $material]) }}" method="POST" enctype="multipart/form-data" class="w-full">
        @csrf
        @method('PUT')
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <input type="text" id="title" name="title" value="{{ old('title', $material->title) }}" required maxlength="255"
                        ...
                        <div>
                            ...
                            <select id="material_type" name="material_type" required
                                ...
                                <option value="presentation" {{ old('material_type', $material->material_type) == 'presentation' ? 'selected' : '' }}>Presentation</option>
                                <option value="handout" {{ old('material_type', $material->material_type) == 'handout' ? 'selected' : '' }}>Handout</option>
                                <option value="video" {{ old('material_type', $material->material_type) == 'video' ? 'selected' : '' }}>Video</option>
                                <option value="document" {{ old('material_type', $material->material_type) == 'document' ? 'selected' : '' }}>Document</option>
                                <option value="other" {{ old('material_type', $material->material_type) == 'other' ? 'selected' : '' }}>Other</option>
                            </select>
                            @error('material_type')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        ...
                        <div>
                            ...
                            <select id="event_session_id" name="event_session_id"
                                ...
                                @foreach($sessions as $session)
                                    <option value="{{ $session->id }}" {{ old('event_session_id', $material->event_session_id) == $session->id ? 'selected' : '' }}>
                        ...
                        <div>
                            ...
                            <input type="number" id="order" name="order" value="{{ old('order', $material->order) }}" min="0"
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="bg-gray-50 rounded-lg p-4">
                            <p class="text-sm font-semibold text-gray-700 mb-1">Current File:</p>
                        ...
                        <div>
                            ...
                            @error('file')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                            <p class="mt-1 text-xs text-gray-500">Leave empty to keep current file. Maximum file size: 50MB</p>
                        </div>
                        ...
                        <div>
                            <label class="flex items-center gap-3">
                                <input type="checkbox" name="is_downloadable" value="1" {{ old('is_downloadable', $material->is_downloadable) ? 'checked' : '' }}
                                    class="w-5 h-5 rounded border-gray-300 text-blue-600 focus:ring-2 focus:ring-blue-500">
                ...
                <div class="lg:sticky lg:top-0 space-y-6">
                    ...
                    <div class="bg-gray-50 rounded-xl border border-gray-200 p-6">
                        ...
                        <div class="space-y-2 text-sm text-gray-600">
                            <div>
                                <p class="text-xs text-gray-500 mb-1">Uploaded At</p>
                            ...
                            <div>
                                ...
                                <p class="text-gray-900">{{ $material->download_count ?? 0 }} times</p>
                            </div>
