...
@section('title', 'Edit TUK Assignment')

@php
    $active = 'events';
@endphp

@section('page_title', 'Edit TUK Assignment')
@section('page_description', $event->code . ' - ' . $event->name)

@section('content')
    <form action="{{ route('admin.events.tuk.update', [$event, $tuk]) }}" method="POST" class="w-full">
        @csrf
        @method('PUT')
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <select id="tuk_id" name="tuk_id" required
                                ...
                                @foreach($availableTuks as $availableTuk)
                                    <option value="{{ $availableTuk->id }}" {{ old('tuk_id', $tuk->tuk_id) == $availableTuk->id ? 'selected' : '' }}>
                                        {{ $availableTuk->code }} - {{ $availableTuk->name }}
                                    </option>
                                @endforeach
                            </select>
                            @error('tuk_id')
                        ...
                        <div>
                            ...
                            <select id="event_session_id" name="event_session_id"
                                ...
                                @foreach($sessions as $session)
                                    <option value="{{ $session->id }}" {{ old('event_session_id', $tuk->event_session_id) == $session->id ? 'selected' : '' }}>
                        ...
                        <div>
                            ...
                            <select id="status" name="status" required
                                ...
                                <option value="pending" {{ old('status', $tuk->status) == 'pending' ? 'selected' : '' }}>Pending</option>
                                <option value="confirmed" {{ old('status', $tuk->status) == 'confirmed' ? 'selected' : '' }}>Confirmed</option>
                                <option value="cancelled" {{ old('status', $tuk->status) == 'cancelled' ? 'selected' : '' }}>Cancelled</option>
                            </select>
                            @error('status')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        ...
                        <div>
                            ...
                            <textarea id="notes" name="notes" rows="4"
                                ...
                                placeholder="Additional notes...">{{ old('notes', $tuk->notes) }}</textarea>
                            @error('notes')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            <label class="flex items-center gap-3">
                                <input type="checkbox" name="is_primary" value="1" {{ old('is_primary', $tuk->is_primary) ? 'checked' : '' }}
                    ...
                    <div class="bg-gray-50 rounded-xl border border-gray-200 p-6">
                        ...
                        <div class="space-y-2 text-sm text-gray-600">
                            <div>
                                ...
                                <p class="text-gray-900">{{ $tuk->created_at->format('d M Y, H:i') }}</p>
                            </div>
                            <div>
                                <p class="text-xs text-gray-500 mb-1">Last Updated</p>
                                <p class="text-gray-900">{{ $tuk->updated_at->format('d M Y, H:i') }}</p>
                            </div>
                            @if($tuk->confirmed_at)
                                <div>
                                    <p class="text-xs text-gray-500 mb-1">Confirmed At</p>
                                    <p class="text-gray-900">{{ $tuk->confirmed_at->format('d M Y, H:i') }}</p>
                                </div>
                            @endif
                        </div>
                    </div>
