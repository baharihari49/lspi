...
@section('title', 'Edit Assessor Assignment')

@php
    $active = 'events';
@endphp

@section('page_title', 'Edit Assessor Assignment')
@section('page_description', $event->code . ' - ' . $event->name)

@section('content')
    <form action="{{ route('admin.events.assessors.update', [$event, $assessor]) }}" method="POST" class="w-full">
        @csrf
        @method('PUT')
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <select id="assessor_id" name="assessor_id" required
                                ...
                                @foreach($availableAssessors as $availableAssessor)
                                    <option value="{{ $availableAssessor->id }}" {{ old('assessor_id', $assessor->assessor_id) == $availableAssessor->id ? 'selected' : '' }}>
                                        {{ $availableAssessor->registration_number }} - {{ $availableAssessor->name }}
                                    </option>
                                @endforeach
                            </select>
                            @error('assessor_id')
                        ...
                        <div>
                            ...
                            <select id="event_session_id" name="event_session_id"
                                ...
                                @foreach($sessions as $session)
                                    <option value="{{ $session->id }}" {{ old('event_session_id', $assessor->event_session_id) == $session->id ? 'selected' : '' }}>
                        ...
                        <div>
                            ...
                            <select id="role" name="role" required
                                ...
                                <option value="lead" {{ old('role', $assessor->role) == 'lead' ? 'selected' : '' }}>Lead Assessor</option>
                                <option value="examiner" {{ old('role', $assessor->role) == 'examiner' ? 'selected' : '' }}>Examiner</option>
                                <option value="observer" {{ old('role', $assessor->role) == 'observer' ? 'selected' : '' }}>Observer</option>
                            </select>
                            @error('role')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        ...
                        <div>
                            ...
                            <select id="status" name="status" required
                                ...
                                <option value="invited" {{ old('status', $assessor->status) == 'invited' ? 'selected' : '' }}>Invited</option>
                                <option value="confirmed" {{ old('status', $assessor->status) == 'confirmed' ? 'selected' : '' }}>Confirmed</option>
                                <option value="rejected" {{ old('status', $assessor->status) == 'rejected' ? 'selected' : '' }}>Rejected</option>
                                <option value="completed" {{ old('status', $assessor->status) == 'completed' ? 'selected' : '' }}>Completed</option>
                            </select>
                            @error('status')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        ...
                        <div>
                            ...
                            <textarea id="notes" name="notes" rows="4"
                                ...
                                placeholder="Additional notes...">{{ old('notes', $assessor->notes) }}</textarea>
                            @error('notes')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <div class="relative">
                                ...
                                <input type="number" id="honorarium_amount" name="honorarium_amount" value="{{ old('honorarium_amount', $assessor->honorarium_amount) }}" min="0" step="1000"
                        ...
                        <div>
                            ...
                            <select id="payment_status" name="payment_status"
                                ...
                                <option value="pending" {{ old('payment_status', $assessor->payment_status) == 'pending' ? 'selected' : '' }}>Pending</option>
                                <option value="paid" {{ old('payment_status', $assessor->payment_status) == 'paid' ? 'selected' : '' }}>Paid</option>
                                <option value="cancelled" {{ old('payment_status', $assessor->payment_status) == 'cancelled' ? 'selected' : '' }}>Cancelled</option>
                            </select>
                    ...
                    <div class="bg-gray-50 rounded-xl border border-gray-200 p-6">
                        ...
                        <div class="space-y-2 text-sm text-gray-600">
                            <div>
                                ...
                                <p class="text-gray-900">{{ $assessor->created_at->format('d M Y, H:i') }}</p>
                            </div>
                            <div>
                                <p class="text-xs text-gray-500 mb-1">Last Updated</p>
                                <p class="text-gray-900">{{ $assessor->updated_at->format('d M Y, H:i') }}</p>
                            </div>
                            @if($assessor->invited_at)
                                <div>
                                    <p class="text-xs text-gray-500 mb-1">Invited At</p>
                                    <p class="text-gray-900">{{ $assessor->invited_at->format('d M Y, H:i') }}</p>
                                </div>
                            @endif
                            @if($assessor->confirmed_at)
                                <div>
                                    <p class="text-xs text-gray-500 mb-1">Confirmed At</p>
                                    <p class="text-gray-900">{{ $assessor->confirmed_at->format('d M Y, H:i') }}</p>
                                </div>
                            @endif
                            @if($assessor->paid_at)
                                <div>
                                    <p class="text-xs text-gray-500 mb-1">Paid At</p>
                                    <p class="text-gray-900">{{ $assessor->paid_at->format('d M Y, H:i') }}</p>
                                </div>
                            @endif
                        </div>
                    </div>
